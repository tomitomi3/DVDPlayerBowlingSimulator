...
namespace Demos
{
    public abstract class Demo : IDisposable
    {
        ...
        /// <summary>時間分解能</summary>
        public float timeResolutionCoeff = 6;
        ...
        public virtual void Update(Window window, Camera camera, Input input, float dt)
        {
            ...
            //Simulation.Timestep(1/FPS, ThreadDispatcher);

            //画面の更新とシミュレーションの時間を独立にしたい場合はどうすればよいか？
            //5
            //Window.Run() while updateHandler -> GameLoop.Update() DemoHarness.Update(dt) -> demo.Update -> Demo.Update
            //画面更新は60fpsだけど、内部は1.0f / (FPS * coeff)分の時間分解能でシミュレーション
            var forwardStepDulation = 1.0f / (FPS * timeResolutionCoeff);
            for (int i = 0; i < timeResolutionCoeff; ++i)
            {
                Simulation.Timestep(forwardStepDulation, ThreadDispatcher);
            }
            ...
            //const float timeToSimulate = 1.0f / FPS;//default 1/60f
            //const int timestepsPerUpdate = 4;
            ...
            //time accumulateを使った方法
            //timeAccumulator += dt;
            //var targetTimestepDuration = 1 / (FPS * timeResolutionCoeff);
            //while (timeAccumulator >= targetTimestepDuration)
            //{
            //    Simulation.Timestep(targetTimestepDuration, ThreadDispatcher);
            //    timeAccumulator -= targetTimestepDuration;
